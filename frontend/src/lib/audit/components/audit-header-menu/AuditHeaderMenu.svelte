<script lang="ts">
	...
	function toggleFilter(columnKey: string, value: string) {
		...
		const idx = auditUiStore.filters.findIndex(f => f.key === filterKey && f.value === filterValue);
		if (idx >= 0) {
			auditUiStore.filters = auditUiStore.filters.filter((_, i) => i !== idx);
		} else {
			auditUiStore.filters = [...auditUiStore.filters, { key: filterKey, value: filterValue }];
		}
...
<div
	...
	class="absolute top-full left-0 z-50 bg-bg-header border border-border-strong rounded shadow-xl py-1 text-sm text-text-primary min-w-48 font-normal normal-case cursor-default text-left flex flex-col"
