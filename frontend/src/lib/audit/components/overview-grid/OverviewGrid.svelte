<script lang="ts">
	...
	import { timeAgo, buildAuditFilters } from './overviewGrid.svelte.ts';
		...
		{#each userStats as stat (stat.userId)}
			...
			<button
				class="rounded-sm px-5 py-4 min-w-64 text-left cursor-pointer
					border-l-3 {hasFlagged ? 'border-l-amber-500 dark:border-l-amber-600' : pct === 100 ? 'border-l-green-500 dark:border-l-green-600' : 'border-l-amber-500 dark:border-l-amber-600'}
					{selectedUser === stat.userId
						? 'bg-blue-50 dark:bg-slate-900 border border-blue-500 dark:border-blue-500 shadow-sm'
						: 'bg-white dark:bg-slate-800 border border-neutral-300 dark:border-slate-600 hover:bg-neutral-50 dark:hover:bg-slate-900'}"
			...
			>
				<div class="flex items-center justify-between mb-3">
					...
					<span class="text-xs font-semibold {hasFlagged ? 'text-amber-600 dark:text-amber-400' : pct === 100 ? 'text-green-600 dark:text-green-400' : 'text-amber-600 dark:text-amber-400'}">{stat.completed}/{stat.total}</span>
				</div>
				<div class="w-full h-1 bg-neutral-200 dark:bg-slate-600 rounded-sm overflow-hidden mb-3 flex">
					{#if flaggedPct > 0}
					...
					<button
						class="px-3 py-1 rounded-sm text-sm font-medium cursor-pointer
							{statusFilter === status
								...
								: 'text-neutral-500 dark:text-neutral-400 hover:bg-white/50 dark:hover:bg-slate-700/50'}"
			...
			{#if auditStore.closedCycles.length > 0}
				<div class="relative" data-panel="cycle-dropdown">
					<button
						...
						class="flex items-center justify-between gap-2 bg-white dark:bg-slate-800 border border-neutral-300 dark:border-slate-600 rounded px-3 py-1 text-sm text-neutral-700 dark:text-neutral-200 cursor-pointer hover:bg-neutral-50 dark:hover:bg-slate-700 min-w-44"
					>
						<span class="truncate">{selectedCycleDate ? formatCycleDate(selectedCycleDate) : 'Select cycle...'}</span>
						<svg class="w-3.5 h-3.5 text-neutral-400 transition-transform flex-shrink-0 {cycleDropdownOpen ? 'rotate-180' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
						</svg>
					</button>
					{#if cycleDropdownOpen}
						<div class="absolute right-0 top-full mt-1 bg-white dark:bg-slate-800 border border-neutral-300 dark:border-slate-700 rounded shadow-xl py-1 text-sm z-50 min-w-52 max-h-64 overflow-y-auto">
							<button
								class="w-full px-3 py-1.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-left cursor-pointer
									{!viewingHistory ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-neutral-500 dark:text-neutral-400'}"
							...
							{#each auditStore.closedCycles as cycle (cycle.id)}
								...
								<button
									class="w-full px-3 py-1.5 hover:bg-blue-50 dark:hover:bg-slate-700 text-left cursor-pointer truncate
										{selectedCycleDate === dateStr ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-neutral-900 dark:text-neutral-100'}"
			...
			<button
				...
				class="px-3 py-1 rounded text-sm font-medium border
					{displayed.length === 0 || csvExporting
						? 'bg-neutral-200 dark:bg-slate-700 text-neutral-400 dark:text-neutral-500 border-neutral-200 dark:border-slate-700 cursor-not-allowed'
						: 'bg-white dark:bg-slate-800 border-neutral-300 dark:border-slate-600 hover:bg-neutral-50 dark:hover:bg-slate-700 text-neutral-700 dark:text-neutral-200 cursor-pointer'}"
						...
						{#each visibleItems as item, i (item.asset_id)}
							...
							>
								{#each OVERVIEW_KEYS as key}
									<div class="flex-1 min-w-0 h-full flex items-center px-2 border-r border-neutral-200 dark:border-slate-700 group-hover:bg-blue-50 dark:group-hover:bg-slate-700 truncate
